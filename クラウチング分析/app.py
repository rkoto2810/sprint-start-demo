...
st.markdown("*※60 fps 以上で撮影した動画をコマ送りしたスクリーンショットを推奨します。*")
...
* ビデオ撮影は60fps以上を推奨(30fps以下ではズレが大きい可能性) 
* セット姿勢で静止しているフレームを選ぶ  
* 体の横から、カメラは腰の高さ  
* 前脚がどちらかを下で選択
        """)
    ...
    if set_file:
        with tempfile.NamedTemporaryFile(delete=False, suffix=".jpg") as tmp:
            tmp.write(set_file.read()); set_path = tmp.name
        st.image(Image.open(set_path), caption="入力画像", use_container_width=True)
...
with tabs[1]:
    ...
    st.image(ASSETS / "ref_drive.jpg",
             caption="参考：60fps以上の動画でコマ送りし、「スネ」が最も倒れたフレームのスクリーンショットを準備する",
             use_container_width=True)
    ...
    if drv_file:
        with tempfile.NamedTemporaryFile(delete=False, suffix=".jpg") as tmp:
            tmp.write(drv_file.read()); drv_path = tmp.name
        st.image(Image.open(drv_path), caption="入力画像", use_container_width=True)
        res = analyze_drive_image(drv_path,"left" if drv_leg=="左" else "right")
        if res:
            st.metric("脛骨角",f"{res['tibia_angle']:.1f}°")
        else: st.error("骨格検出に失敗しました。")

# ─────────────────────────────────── ③ Air
with tabs[2]:
    st.header("③ Air 局面")
    st.image(ASSETS / "ref_air.jpg",
             caption="参考：60fps以上の動画でコマ送りし、股関節が一番伸びきった曲面のフレームのスクリーンショットを準備する",
             use_container_width=True)
    st.markdown(
        "進行方向を指定（不明な場合は **自動判定** で OK）して画像をアップロードしてください。")
    air_dir = st.radio("進行方向", ("右向き","左向き","自動判定"))
    air_file= st.file_uploader("Air 画像", ["jpg","jpeg","png"], key="air")

    if air_file:
        with tempfile.NamedTemporaryFile(delete=False,suffix=".jpg") as tmp:
            tmp.write(air_file.read()); air_path=tmp.name
        st.image(Image.open(air_path), caption="入力画像", use_container_width=True)
...
with tabs[3]:
    ...
    st.image(ASSETS / "ref_landing.jpg",
             caption="参考：60fps以上の動画でコマ送りし、1 歩目着地直後のフレームのスクリーンショットを準備する",
             use_container_width=True)
    st.markdown(
        """
**評価項目**  
...
* 脛 - 体幹差（≤5°）  
    ...
    if land_file:
        with tempfile.NamedTemporaryFile(delete=False,suffix=".jpg") as tmp:
            tmp.write(land_file.read()); land_path=tmp.name
        st.image(Image.open(land_path), caption="入力画像", use_container_width=True)
