...
st.caption("※60 fps 以上の動画からコマ送りしたスクリーンショットを推奨します。")
...
* 前脚がどちらかを下で選択
    ...
    if set_file:
        with tempfile.NamedTemporaryFile(delete=False, suffix=".jpg") as tmp:
            ...
            set_path = tmp.name
        st.image(Image.open(set_path), caption="入力画像", use_column_width="always")
...
with tabs[1]:
    ...
    if drv_file:
        with tempfile.NamedTemporaryFile(delete=False, suffix=".jpg") as tmp:
            ...
            drv_path = tmp.name
        st.image(Image.open(drv_path), caption="入力画像", use_column_width="always")
...
with tabs[2]:
    ...
    st.image(ASSETS / "ref_air.jpg",
             ...
             use_column_width="always")
    st.markdown("進行方向を指定（不明な場合は **自動判定**）して画像をアップロードしてください。")
    ...
    if air_file:
        with tempfile.NamedTemporaryFile(delete=False, suffix=".jpg") as tmp:
            ...
            air_path = tmp.name
        st.image(Image.open(air_path), caption="入力画像", use_column_width="always")
...
* 脛 − 体幹差（≤5°）
    ...
    if land_file:
        with tempfile.NamedTemporaryFile(delete=False, suffix=".jpg") as tmp:
            ...
            land_path = tmp.name
        st.image(Image.open(land_path), caption="入力画像", use_column_width="always")
